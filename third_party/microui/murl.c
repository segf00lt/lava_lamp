...
void murl_render(mu_Context *ctx) {
  ...
  while (mu_next_command(ctx, &cmd)) {
    ...
    case MU_COMMAND_ICON: {
      ...
      int compensate = 2;
      mu_Rect rect_compensate_size = {
          .x = cmd->icon.rect.x + compensate,
          .y = cmd->icon.rect.y + compensate,
          .w = cmd->icon.rect.w - 2*compensate,
          .h = cmd->icon.rect.h - 2*compensate,
      };
      DrawText(icon, rect_compensate_size.x, rect_compensate_size.y, rect_compensate_size.h,
               icon_color);
    } break;
